...
if ( $wp_query->have_posts() ) {
    while ( $wp_query->have_posts() ) {
        ...
        // 処理を記述
			...
			$image .= '<div class="work">' .get_the_post_thumbnail() .'</div>';
		}else{
...
// 投稿データのリセット</p>
wp_reset_postdata();
	return  '<div class="works">' . $image . '</div>' . $str ;
}}
...
if ( $wp_query->have_posts() ) {
    while ( $wp_query->have_posts() ) {
        ...
        // 処理を記述
			if(has_post_thumbnail()){
			$image .= '<div class="re-image">' .get_the_post_thumbnail() .'</div>' .'<h3>' . get_field('recruit_gate') .'</h3>';
		}else{
			$image .= '<div class="work"><div class="js-modal-open" data-modal="' . get_post_field( 'post_name', get_the_ID() ) . '"><div class="img"><img src="./wp-content/themes/twentynineteen_child/images/no-image.png"></div></div></div>';
		};
...
// 投稿データのリセット
wp_reset_postdata();
	return  '<div class="re-image">' . $image . '</div>';
}}
